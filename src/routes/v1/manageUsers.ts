...
router.post("/user", async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (typeof req.body.username !== "string") return res.status(400).send("Invalid username");
        if (typeof req.body.password !== "string") return res.status(400).send("Invalid password");
        if (typeof req.body.permissions !== "object") return res.status(400).send("Invalid permissions");
...
router.patch("/user/me", async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.user) return res.status(401).send("Unauthorized");
        if (!req.body.username) return res.status(400).send("Missing username");
        if (typeof req.body.username !== "string") return res.status(400).send("Invalid username");
...
router.delete("/user/me", async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.user) return res.status(401).send("Unauthorized");
